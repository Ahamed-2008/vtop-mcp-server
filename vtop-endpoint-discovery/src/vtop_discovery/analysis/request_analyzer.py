...
def infer_parameter_schema(
    ...
    examples = [val_str] if val_str and val_str != "[REDACTED]" else []
    ...
    if name in _TIMESTAMP_NAMES or (val_str.isdigit() and len(val_str) in (10, 13)):
        return ParameterSchema(
            ...
            dynamic=True,
            required=False,
            description="Epoch timestamp or cache buster",
        )
    ...
    if lowered in {"username", "user", "uname", "loginid"}:
        return ParameterSchema(
            ...
            dynamic=False,
            required=True,
            description="User login handle or registration number",
        )
    ...
    if "captcha" in lowered:
        return ParameterSchema(
            ...
            dynamic=False,
            required=True,
            description="Solved CAPTCHA verification string",
        )
    ...
    if any(k in lowered for k in _COURSE_NAMES):
        return ParameterSchema(
            ...
            dynamic=False,
            required=True,
            description="Course or class registration code",
        )
    ...
    return ParameterSchema(
        ...
        dynamic=False,
        required=True,
        description="Query or form parameter",
    )
...
def analyze_request_parameters(endpoint: Endpoint) -> None:
    ...
    for name, val in endpoint.request.query.items():
        existing = endpoint.request.parameters.get(name)
        schema = infer_parameter_schema(name, val, location="query")
        if existing and existing.examples:
            for ex in existing.examples:
                if ex not in schema.examples:
                    schema.examples.append(ex)
        schemas[name] = schema
    ...
    for name, val in body_dict.items():
        existing = endpoint.request.parameters.get(name)
        schema = infer_parameter_schema(name, val, location="form")
        if existing and existing.examples:
            for ex in existing.examples:
                if ex not in schema.examples:
                    schema.examples.append(ex)
